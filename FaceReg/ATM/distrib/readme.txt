    ...
    B. Set the path by doing one of the following:
        ...
        * Add the MCR directory to the environment variable by issuing 
        ...
        NOTE: When deploying C and C++ standalone applications, it is 
              possible to run the shell script file run_ATM.sh 
...
3. Collect the following files, based on what you are deploying: 
    ...
    All Platforms:
        * ATM.ctf (If present)(Component Technology File archive,
          platform-dependent on target machine)

    Windows:
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          installer)
        * ATM.exe (application created by MATLAB Compiler)

    UNIX:
        * MCRInstaller.bin (MATLAB Compiler Runtime installer)
        * ATM (application created by MATLAB Compiler)
        * run_ATM.sh (shell script that can be run to 
    ...
    All Platforms:
        * ATM.dll (shared library, file extension varies by 
          platform)
        * ATM.h (library header file)
        * ATM.ctf (If present)(Component Technology File archive,
    ...
    Windows:
        * ATM.ctf (Component Technology File archive, contains 
          compiled M-code)    
        * ATM.xml (documentation files)
        * ATM.pdb (program database file - if DEBUG option 
          selected)
        * ATM.dll (component assembly file)
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
    ...
    All Platforms: 
        * ATM.jar (Java class files and compiled M-code)
    ...
    Windows:
        ...
        * ATM.ctf (Component Technology File archive, contains 
          compiled M-code)
        * ATM.xla (Optionally created and placed in
          C:\xampp\htdocs\ATM\FaceReg\ATM\src directory by the user)  
        * MCRInstaller.exe (self-extracting MATLAB Compiler Runtime
          archive)
